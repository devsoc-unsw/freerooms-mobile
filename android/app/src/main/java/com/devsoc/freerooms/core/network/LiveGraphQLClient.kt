...
class LiveGraphQLClient(private val apolloClient: ApolloClient) : GraphQLClient {
    override suspend fun <D : Query.Data> query(query: Query<D>): NetworkResult<D> {
        ...
        return try {
            ...
            Log.d(
                "LiveGraphQLClient",
                "Response received. " +
                    "hasData: ${response.data != null}, errors: ${response.errors?.size ?: 0}"
            )

            if (response.exception != null) {
                Log.e(
                    "LiveGraphQLClient",
                    "Apollo Exception: ${response.exception?.message}",
                    response.exception
                )
            }
            ...
            if (data != null) {
                ...
                response.errors?.forEach { Log.e("LiveGraphQLClient", "Detailed Error: ${it.message}") }

                NetworkResult.Error(errorMessage, response.exception)
            }
        } catch (e: ApolloException) {
            Log.e("LiveGraphQLClient", "Apollo Network/Parsing Exception", e)
